...
locals {
  ...
  app_environment_filename   = "sic-code-api.env"
  ...
  vpc_name = data.aws_ssm_parameter.secret[format("/%s/%s", local.name_prefix, local.vpc_name)].value
